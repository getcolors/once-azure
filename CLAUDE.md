...
Desired-state deployment for one ONCE server on Azure. `colors.yml` is source;
`.colors/` is generated and must never be edited or committed. The OpenTofu
backend is local, so generated state must not be deleted while the deployment
exists.
...
Build and dry-run require no credentials. Real provider operations use Azure
CLI authentication plus `COLORS_PAR_*` secrets loaded from the ignored
